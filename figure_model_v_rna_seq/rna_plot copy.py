...
# logi = np.isin(cell_of_string_1, cell_of_string_2)

# loca = [cell_of_string_2.index(item) if item in cell_of_string_2 else -1 for item in cell_of_string_1]
...
sns.scatterplot(data=df, x=col_x, y=col_y, hue=key_column, palette="Set1", s=100)
...
for group, stats in group_stats.iterrows():
    # Plot median
    plt.scatter(stats['median_x'], stats['median_y'], color='black', zorder=5)
    
    # Plot 10-90% range as a rectangle
    plt.plot([stats['lower_x'], stats['upper_x']], [stats['median_y'], stats['median_y']], color='black', lw=2)
    plt.plot([stats['median_x'], stats['median_x']], [stats['lower_y'], stats['upper_y']], color='black', lw=2)

# Calculate the Pearson correlation across all data
corr, p_value = pearsonr(df[col_x], df[col_y])

# Set the title with the Pearson correlation
plt.title(f"Pearson Correlation: {corr:.2f}", fontsize=16)
